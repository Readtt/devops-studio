...
/**
 * Persistent suite-chat thread browser. Lists every saved thread, newest-updated
 * first. Since a suite can host multiple parallel threads, each thread gets its
 * own row. The thread's title (auto-derived from the first user message or set
 * explicitly in the chat pane) is the primary label; the suite/plan path is the
 * subtitle. (Commit Review history lives in the History tab, not here.)
 */
