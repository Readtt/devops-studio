...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S devops-studio-bin";
    case "debian":
      return `sudo apt install ./DevOpsStudio_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./DevOpsStudio-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `DevOps Studio v${manual.version} is available`
                  : `DevOps Studio v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart DevOps Studio to finish installing."
              : downloading
                ? progress !== null
