...
function getRecycler(): HTMLDivElement {
  ...
  el.setAttribute("data-ds-recycler", "");
  el.style.cssText =
...
function createSlot(): Slot {
  ...
  host.setAttribute("data-ds-slot", String(slots.length));
...
function bindSlot(slot: Slot, p: AcquireParams): void {
  ...
  if (p.snapshot) {
    try {
      ...
      console.warn("[devops-studio] snapshot replay failed:", e);
    }
  }
  if (p.altScreen) {
...
function serializeSlot(slot: Slot): SerializeOutput {
  ...
  try {
    ...
    console.warn("[devops-studio] serialize failed:", e);
  }
  return {
    snapshot,
    cols: slot.term.cols,
...
function attachWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[devops-studio-webgl] unavailable:", e);
  }
}
...
function disposeSlotWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[devops-studio-webgl] dispose failed:", e);
  }
  try {
    const r = (
