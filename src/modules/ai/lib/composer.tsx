...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("devops-studio:ai-attach-file", onAttach);
    return () => window.removeEventListener("devops-studio:ai-attach-file", onAttach);
  ...
  const submit = () => {
    ...
    // the prompt to the DEVOPS_STUDIO.md scan template before sending.
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<devops-studio-command name="${outcome.commandName}" />`;
        }
      }
    }
