...
import { GeneratorStack } from "@/modules/generator";
...
export default function App() {
  ...
  // cycleSidebarView removed in v1: source-control is hidden and the remaining
  // rail buttons (Files / Test Plans / Stale) are persistSidebarView-only.
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if ((e.metaKey || e.ctrlKey) && k === "k") {
        // Don't fight terminal Cmd+K (clear scrollback) — only intercept
        // when no input is focused that already handles it.
        const tag =
  ...
  // Kick off one staleness scan a few seconds after launch — populates the
  // status-bar "Stale: N" badge so the user doesn't have to open the panel
  // to discover pending review work. Non-blocking, fails silently.
  useEffect(() => {
  ...
  const isTestCaseTab = activeTab?.kind === "test-case";
  const isGeneratorTab = activeTab?.kind === "generator";
  ...
  const handleAttachFileToAgent = useCallback(
    (path: string) => {
      ...
      window.dispatchEvent(
        new CustomEvent<string>("devops-studio:ai-attach-file", { detail: path }),
      );
      openPanel();
      focusInput(null);
    },
  ...
  // Source-control sidebar is hidden in DevOps Studio v1; git diff/history
  // tabs still keep the source-control hook alive when open.
  const sourceControlActive = hasOpenGitTab;
  ...
  // Source-control sidebar is hidden in v1 — pane.source shortcut becomes a no-op.
  const toggleSourceControl = useCallback(() => {
    /* noop in DevOps Studio v1 */
  }, []);
  ...
  const workspaceSurface = (
    <div className="relative h-full min-h-0">
      ...
      <div
        className={cn(
          "absolute inset-0",
          !isTestCaseTab && "invisible pointer-events-none",
        )}
        aria-hidden={!isTestCaseTab}
      >
        <TestCaseStack tabs={tabs} activeId={activeId} />
      </div>
      <div
        className={cn(
          "absolute inset-0",
          !isGeneratorTab && "invisible pointer-events-none",
        )}
        aria-hidden={!isGeneratorTab}
      >
        <GeneratorStack
          tabs={tabs}
          activeId={activeId}
