...
The bottom status bar shows — and **switches** — the current git branch of the user's source directory (`usePreferencesStore.sourceRoot`). Reading is `src/modules/git/useSourceDirGitInfo.ts` (branch/commit) and `useSourceDirStatus.ts` (dirty + ahead/behind + `parkedHere`, via Rust `git_status_summary`), both polled every 30 s, on window focus, and on the `source-git-changed` event. The switcher lives in `src/modules/git/StatusBarGit.tsx`: it lists branches via `git_branches` (structured: local + remote-only, deduped, current-first, recency-sorted); picking one runs checkout → fast-forward pull (`git_ops.rs`), driven by `useBranchSwitch.ts` and surfaced through a glass-capsule toast (`BranchSwitchToast.tsx`, matched 1:1 to the updater toast). Pull is fast-forward-only and never auto-merges/rebases.

The dirty-tree flow follows GitHub Desktop's model (researched): `BranchSwitchDialog.tsx` offers **Bring my changes to X** (plain checkout — git carries them or refuses, never conflict-markers) vs **Leave my changes on Y** (stash tagged with `PARKED_STASH_MARKER`; the target opens clean — we never auto-reapply). Restore is **manual, not auto-on-return**: when a branch has parked changes (`parkedHere`), the switcher shows a "Restore changes you left here" item (and a pill indicator) that runs `git_stash_restore` — `apply` then `drop`-only-on-clean, so a conflicting restore keeps the stash (conflicts surfaced by unmerged paths, never by message-scraping). Picking a **remote-only** branch creates a local tracking branch (`checkout -t`). A manual **Fetch from remote** button (`git_fetch --prune`, no auto-fetch — avoids credential-prompt fatigue) refreshes the list with a "last fetched" hint.

Code links on published cases **always** track the live source-dir branch: the generator resolves the `$current` sentinel at publish-time from the working-dir branch (falling back to `main` only on a detached HEAD / non-repo). There is no fixed-branch option — `AzureDevOpsSection` shows a read-only explainer, and `defaultTrackingBranch` is always saved as `$current` (the generator ignores any legacy fixed value). The separate per-run **"Tag with source branch"** switch still controls whether any branch/commit provenance is stamped at all.
...
- **Branch pickers use `BranchPicker`** (`src/components/BranchPicker.tsx`), not a raw `<Select>`. It's a cmdk Combobox in a Popover with fuzzy search and `max-h-[280px]` so long branch lists don't take the viewport. The generator's developer picker consumes it; new setting/form branch-picker surfaces should too. (The status-bar branch *switcher* in `StatusBarGit.tsx` builds its own cmdk list rather than `BranchPicker` because its trigger is the status-bar pill segment, not a standalone control.)
