...
The switcher lives in `src/modules/git/StatusBarGit.tsx`. At **one** repo it is exactly what it always was: folder segment + branch pill. At **more than one** the folder segment shows `3 repos` and opens Settings (the directory picker writes through `setSourceRoot`, which would collapse the workspace to the one folder picked), and the branch segment summarises the repos — the branch when they all agree, else `N branches` — and opens a repo list that drills into one repo's branch list with `cwd={repo.root}`. That repo list's footer carries **Add folder…** and **Get source code…** — the clone wizard used to hang off the "Get source code" segment alone, which only shows when NO repo is configured, so it vanished the moment a tester had one. Every entry point says *Get source code*, matching `GetSourceCodeDialog`/`GetSourceCodeButton`; "clone" stays the verb in prose and in the `cloneProgressStore`/`cloneRepo` plumbing, never a label. The wizard itself is hosted by `StatusBarGit` rather than either switcher, so it opens at any repo count — including on request from the **Settings** window, whose Source-repos block has its own `Get source code…` that emits `devops-studio://get-source-code-requested` and focuses main. Settings deliberately does not host a second copy: `cloneProgressStore` and its capsule are per-window, and two independent batches would interleave their `addRepo` read-modify-writes. Branches come from `git_branches` (structured: local + remote-only, deduped, current-first, recency-sorted); picking one runs checkout → fast-forward pull (`git_ops.rs`), driven by `useBranchSwitch.ts` and surfaced through a glass-capsule toast (`BranchSwitchToast.tsx`, matched 1:1 to the updater toast). Pull is fast-forward-only and never auto-merges/rebases.
...
A workspace repo carries an optional `ado: { repoId, repoName, project }` binding — the Azure DevOps repository a published code link deep-links into. It is resolved in `src/modules/ado/repoBinding.ts` from `git config --get remote.origin.url` (surfaced as `GitRepoInfo.remoteUrl`) against the **org-wide** `ado_list_repos`, in three tiers: normalised remote-URL equality → same `{project, repo}` parsed out of either URL shape (SSH and legacy `visualstudio.com` remotes never string-match the HTTPS one) → a folder-basename match that must be **unique across the org**, because a name two projects share would bind the wrong project. Nothing else binds: an unmatched repo stays unlinked and Settings' repo row offers "Set ADO repo…" (a cmdk picker grouped by project) and "Detect from remote". `autoBindRepos` runs after every add site (Settings, the status-bar picker, the clone batch) and skips repos that already have a binding, so it costs one REST call per add, not per settings visit. It **also sweeps the whole registry once per session** — when the ADO connection first reads as configured, and again on `onAdoConnectionChanged` (`App.tsx`), because a new org can bind what the old one couldn't. Without that sweep the only repos ever offered to the binder were the ones that passed an add site, so the repo **migrated from the pre-registry `sourceRoot`** (and any folder opened via the shell verb) stayed unlinked forever while every later clone bound normally.

Published links therefore carry the **bound repo's** ADO name, id and project — not the workspace folder name, and not the connection's project, which is only correct for repos that happen to live in it. `SourceLink.project` and the block's `project:` key are **optional on purpose**: `sourceLinksParser` drops a line missing a required key, so making it required would erase the links on every case published before this existed. `TestCasePane` reads the link's own project and falls back to the connection's. This fixes the **web URL** only — `ado_get_file`, diffs and branches are still project-scoped to the connection (`project_api`), so cross-project *file reads* remain unimplemented.
