...
Code links on published cases **always** track the live source-dir branch, read from the working dir at publish-time. When no branch resolves (detached HEAD / non-repo) the link records no branch at all rather than a fabricated `main`; a detached HEAD still stamps its commit. There is no fixed-branch option — `AzureDevOpsSection` shows a read-only explainer, and `defaultTrackingBranch` is always saved as `$current` (the generator ignores any legacy fixed value). The separate per-run **"Tag with source branch"** switch still controls whether any branch/commit provenance is stamped at all.
