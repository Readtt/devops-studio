# devops-studio-shell-integration (zshrc)
#
...
# zsh, so we shadow $? into `_ds_ret`.

{
  _ds_user_zdotdir="${DEVOPS_STUDIO_USER_ZDOTDIR:-$HOME}"
  [ -f "$_ds_user_zdotdir/.zshrc" ] && source "$_ds_user_zdotdir/.zshrc"
...
if [[ -z "$__DS_HOOKS_LOADED" ]]; then
  ...
  _ds_precmd() {
    local _ds_ret=$?
    printf '\e]133;D;%s\e\\' "$_ds_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_ds_urlencode "$PWD")"
  ...
  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _ds_precmd
    add-zsh-hook preexec _ds_preexec
  fi
