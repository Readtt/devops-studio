...
if [ -z "$__DS_HOOKS_LOADED" ]; then
  ...
  _ds_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_ds_urlencode "$PWD")"
    if [ -z "$__DS_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __DS_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_ds_precmd:"*) ;;
    *) PROMPT_COMMAND="_ds_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
